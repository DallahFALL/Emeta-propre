 ...
 * FICHIER : script.js (Version Auditée - Monitoring & Widget Live)
 */

// --- CONFIGURATION ---
const WEBHOOK_URL = "https://hook.eu2.make.com/5414bbls1n5au1ebf0qhlx6htdeitaxp"; 
// Note : Pour le widget, vous pouvez utiliser un Webhook de lecture dédié ou le même si configuré avec un routeur.
const STATS_WEBHOOK_URL = "https://hook.eu2.make.com/votre_webhook_de_lecture"; 
...
function validateStep2() {
    ...
    if (!sector || !geo) { 
        alert(document.documentElement.lang === 'fr' ? "Complétez la matrice stratégique." : "Complete the strategic matrix."); 
        return false; 
    }
    return true;
}

// --- MONITORING LIVE (WIDGET STATS) ---
function updateLiveStats() {
    fetch(STATS_WEBHOOK_URL)
        .then(response => response.json())
        .then(data => {
            if (document.getElementById('count-analyses')) 
                document.getElementById('count-analyses').innerText = data.totalAnalyses || "--";
            if (document.getElementById('count-pdf')) 
        ...
        .catch(err => console.warn("Widget Stats en attente de flux..."));
}

// --- TÉLÉCHARGEMENT PDF & SIGNAL MONITORING ---
function downloadPDF() {
    const element = document.getElementById('resultBody');
    const companyName = document.getElementById('company').value || "e-META-LABS";
    
    // Monitoring silencieux du téléchargement
    fetch(WEBHOOK_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: "PDF_DOWNLOAD", company: companyName, timestamp: new Date().toISOString() })
    ...
    printWindow.document.write(`
        <html>
            ...
            <body>
                <h1>e-META LABS</h1>
                ${element.innerHTML}
                <div class="footer"><p>Certifié par horodatage Blockchain 2026</p></div>
            </body>
        </html>
    `);
...
document.addEventListener('DOMContentLoaded', () => {
    updateLiveStats();
    setInterval(updateLiveStats, 60000); // Refresh stats toutes les minutes
